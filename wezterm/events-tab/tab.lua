...
local utils = require("events-tab.utils")
...
wezterm.on("format-tab-title", function(tab, _tabs, _panes, _config, hover, max_width)
  ...
  local title = set_info(tab.active_pane, 1)
  ...
  if CONFIG.use_icons then
    ...
    return string.format("%s%s %d %s%s", pad, icon, index, title, pad)
  end
...
function set_info(pane)
  return clean_process_name(pane.foreground_process_name)
end
